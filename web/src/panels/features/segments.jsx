      ...
      <Section title={`${TXT.clusters} (${d.total})`}>
        {d.segments.map((s, i) => (
          <Card key={i} className="mb-2">
            ...
            <div className="flex flex-wrap gap-1">{s.members.map(m => <button key={m.id} onClick={() => showPanel({ view: 'customer', customer_id: m.id })} className="text-[12px] bg-bg-2 rounded-md px-2 py-0.5 hover:bg-bg-3">{m.name}</button>)}</div>
          </Card>
        ))}
      </Section>
    </>
